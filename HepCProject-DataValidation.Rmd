...
setwd("Z:/Hepatitis C Project/RawData/TextFormat")
library(readr)
...
##########Encounters File########
#encounters <- read_delim("Z:/RawData/TextFormat/Encounters.txt", "|", escape_double = FALSE, trim_ws = TRUE)
encounters <- read_delim("Z:/Hepatitis C Project/RawData/TextFormat/Encounters.txt", "|", escape_double = FALSE, trim_ws = TRUE)#Cindy's path - 647705 observations
encounters <- read_delim("Z:/Hepatitis C Project/RAW NEW 3.10.2017/2017 03 10/Enc.txt", "|", escape_double = FALSE, trim_ws = TRUE)#Cindy & Laura's path for new data files
...
####HH do type conversions for certain fields that need cleanup

encounters$Zip_Code_DID<-as.factor(encounters$Zip_Code_DID) #is this conversion needed?
encounters$PCP_Prov_ID<-as.character(encounters$PCP_Prov_ID,width=5) #is this conversion needed?
...
```{r}
##########Meds_Inpt File########
meds_inpt <- read_delim("Z:/Hepatitis C Project/RAW NEW 3.10.2017/2017 03 10/Meds_Inpt.txt", "|", escape_double = FALSE, trim_ws = TRUE)#Laura's path for new data files
...
meds_inpt$Medication_id <- as.character(meds_inpt$Medication_id)

#Medication_id
sum(is.na(meds_inpt$Medication))
meds_inpt$Medication <- as.character(meds_inpt$Medication)
